...
if (! class_exists('WC_Shipping_Shipcloud_Webhook')) {
	...
	class WC_Shipping_Shipcloud_Webhook
	{
		...
		private function process_shipment_event($event)
		{
			...
			$order_id = $order !== null ? $order->get_id() : null;
			if (empty($order_id)) {
			...
			$order->update_meta_data('shipment_' . $shipment_id . '_status', $event->type);
			$order->save();
			...
			if (! empty($shipment)) {
				...
				WC_Shipping_Shipcloud_Utils::update_shipment_for_order($order_id, $shipment_data);
				...
				if (! empty($tracking_event)) {
					...
					$order->add_meta_data('shipment_' . $shipment_id . '_trackingevent', $meta);
					$order->save();
				} else {
		...
		private function delete_shipment_from_database($order_id, $shipment_id, $event)
		{
			$order 		= WC_Shipping_Shipcloud_Utils::find_order_by_shipment_id($shipment_id);
			if ($order == null) {
				$this->log('Order not found for shipment id ' . $shipment_id, 'error');
				return;
			}
			...
			foreach ($shipments as $_shipment) {
				...
				if ($_shipment_data['id'] == $shipment_id) {
					$order->delete_meta_data_by_mid($_shipment_meta_id);
					$order->delete_meta_data_value('shipcloud_shipment_ids', $shipment_id);
					$order->save();
