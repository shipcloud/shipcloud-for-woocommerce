	...
	class WC_Shipping_Shipcloud_Utils
	{
		...
		public static function find_order_by_shipment_id($shipment_id)
		{
			...
			foreach ($orders as $order) {
				if (self::get_shipment_for_order($order->get_id(), $shipment_id)) {
		 ...
		 * Returns [ 'id' => 'shipment_id', 'carrier_tracking_no' => 'tracking_no', 'tracking_url' => 'tracking_url' ]
		 * 
		 * @param $order_id
		 * @param $shipment_id
		 * @return array
		...
		// TODO: Find where get_order_meta_shipment_data() is used, because that might do an update to meta.
		public static function get_order_meta_shipment_data($order_id, $shipment_id)
		{
			$wc_order = wc_get_order($order_id);
			$shipments = $wc_order->get_meta('shipcloud_shipment_data', 0);
			...
			foreach ($shipments as $shipment) {
				$data = $shipment->get_data()['value'];
				if (isset($data['id']) && $data['id'] === $shipment_id) {
					return $data;
				}
			}
		...
		public static function convert_to_wc_api_response($shipment, $order_id = null)
		{
			...
			if ($order_id) {
				$shipment_status_meta = wc_get_order($order_id)->get_meta('shipment_' . $shipment['id'] . '_status');
				// Checked for HPOS compatibility
				$data['shipment_status'] = self::get_status_string($shipment_status_meta);
		...
		public static function format_price($price = 0)
		{
			...
			// Check if the price string contains the current currency
			if (strpos($price, ' ' . $currency) !== false) {
				// If the currency is found, return the price as it is
				return $price;
			}
		...
		/**
		 * Updates the shipment data
		 * Added for HPOS compatibility
		 * 
		 * @param int $order_id
		 ...
		 * @return void
		 */
		public static function update_shipment_for_order(int $order_id, array $shipment_data)
		{

			if (!isset($shipment_data['id'])) {
				return false;
			}
			...
			foreach ($_shipments as $_shipment) {
				$_shipment_data = $_shipment->get_data()['value'];

				$_shipment_meta_id = $_shipment->id;
				if ($_shipment_data['id'] == $shipment_id) {
					$wc_order->update_meta_data('shipcloud_shipment_data', $shipment_data, $_shipment_meta_id);
					$wc_order->save();
					return true;
				}
			}
		...
		public static function get_parcel_description($order)
		{
			...
			$shipment = $order->get_meta('shipcloud_shipment_data', true);
			...
			if (isset($shipment['description'])) {
				return $shippinshipmentg_data['description'];
			}
