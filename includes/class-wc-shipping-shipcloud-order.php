...
if (! class_exists('WC_Shipping_Shipcloud_Order')) {
	...
	class WC_Shipping_Shipcloud_Order
	{
		 ...
		 * @return string
		 */
		public function create_shipment_center($post_or_order)
		{
			global $post, $woocommerce;


			if (! class_exists('Automattic\WooCommerce\Utilities\OrderUtil')) {
				include_once(ABSPATH . 'wp-content/plugins/woocommerce/src/Utilities/OrderUtil.php');
			}

			if (OrderUtil::custom_orders_table_usage_is_enabled()) {

				// Ensure OrderUtil class is included
		...
		private function add_care_of_input_field($data, $category)
		{
			...
			if (array_key_exists('show_recipient_care_of', $options)) {
				if (
					'in_' . $category === $options['show_recipient_care_of'] ||
		 ...
		 * @return void
		 */
		public function display_tracking_information($wc_order)
		{
			...
			if ($show_tracking_in_my_account === 'yes') {
				...
				// WOOCOMMERCE HPOS COMPATIBILITY
				$shipment_ids 	= $wc_order->get_meta('shipcloud_shipment_ids');
				$shipments_data = $shipment_data = $wc_order->get_meta('shipcloud_shipment_data', 0);
				ob_start();
		...
		public function create_shipment($data)
		{
			...
			if (! $wc_order) {
				return new \WP_Error(1, sprintf('No matching order found for order ID %s', $order_id));
			}
			...
			try {
				...
				if (isset($shipment_id)) {
					...
					if (is_wp_error($shipment)) {
						error_log('Error updating shipment: ' . $shipment->get_error_message());
						return $shipment;
					}
					...
					$wc_order->save();
					if (
						array_key_exists('customs_declaration', $prev_shipment_data)
					...
					if (is_wp_error($shipment)) {
						error_log('Error creating shipment: ' . $shipment->get_error_message());
						return $shipment;
					}
		...
		private function extract_pickup_time($data, $method = null)
		{
			...
			if (
				empty($data['pickup']['pickup_earliest_date'])
			...
			try {
				...
				if (is_wp_error($pickup_request)) {
					...
					if (! empty($shipment_id)) {
						...
						// Checked for HPOS compatibility:
						// This should work for both HPOS and Legacy. Untested.
						$_shipments = $wc_order->get_meta('shipcloud_shipment_data', 0);
						foreach ($_shipments as $_shipment) {
							$_shipment_data = $_shipment->get_data();
							$_shipment_meta_id = $_shipment->id;
							if ($_shipment_data['id'] == $shipment_id) {
								$updated_shipment = array_merge(
									$_shipment_data,
									['pickup_request' => $pickup_request]
								);

								$wc_order->update_meta_data('shipcloud_shipment_data', $updated_shipment, $_shipment_meta_id);
								$wc_order->save();
						...
						// error_log( 'shipments HPOS: ' . print_r( $shipments, true ) );
						// $shipments = get_post_meta( $order_id, 'shipcloud_shipment_data' );
						// error_log( 'shipments Legacy: ' . print_r( $shipments, true ) );
						...
						// Checked for HPOS compatibility:
						// HPOS Compatible variant. Should work, untested.
						$_shipments = $wc_order->get_meta('shipcloud_shipment_data', 0);

						foreach ($_shipments as $_shipment) {
							$_shipment_data = $_shipment->get_data();
							$_shipment_meta_id = $_shipment->id;
							$shipment_id = $_shipment_data['id'];
							if (in_array($shipment_id, $ids)) {
								$updated_shipment = array_merge(
									$_shipment_data,
									['pickup_request' => $pickup_request]
								);
		...
		private function delete_shipment_from_db($shipment_id)
		{
			...
			foreach ($_shipments as $_shipment) {
				...
				$_shipment_meta_id = $_shipment->id;
				if ($_shipment_data['id'] == $shipment_id) {
					$wc_order->delete_meta_data('shipcloud_shipment_data', $_shipment_meta_id);
					// add note
					$wc_order->add_order_note(__('shipcloud shipment has been deleted.', 'shipcloud-for-woocommerce'));
					$wc_order->save();
					return true;
				}
			...
			foreach ($data as $additional_service_key => $additional_service_value) {
				...
				switch ($additional_service_key) {
					case 'visual_age_check':
						if (
							array_key_exists('age_based_delivery', $data)
					...
					case 'gls_guaranteed24service':
						if (
							array_key_exists('checked', $data['gls_guaranteed24service'])
							&&	array_key_exists('checked', $additional_service_value)
						) {
							$additional_services[] = array(
		...
		public function email_notification_enabled()
		{
			// Checked for HPOS compatibility:
			// This should work for both HPOS and Legacy. Untested.
			$notification_enabled = $this->get_wc_order()->get_meta('shipcloud_parcel_delivery_notification', true);
