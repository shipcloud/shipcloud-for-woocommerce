...
<div id="shipment-center">
	...
	<?php /* Addresses */ ?>
	<div class="section addresses <?php if ($order_status === 'completed') {
										echo 'hidden';
									} ?>">
			...
			<div class="address-form recipient disabled">
				...
				<p class="fullsize">
					...
					<input type="text"
						...
						value="<?php if (array_key_exists('state', $recipient)): echo $recipient['state'];
								endif ?>"
						disabled>
				</p>
	...
	<?php /* Parcel Content */ ?>
	<div class="section parcels  <?php if ($order_status === 'completed') {
										echo 'hidden';
									} ?>" data-calculated-weight="<?php echo ($this->get_calculated_weight()); ?>">
	...
	if (OrderUtil::custom_orders_table_usage_is_enabled()) {
		$shipment_meta = $order->get_meta('shipcloud_shipment_data', 0);
		if($shipment_meta && is_array($shipment_meta)){
			$shipment_data = $shipment_meta;
		}else if ($shipment_meta && $shipment_meta->get_data()) {
			$shipment_data = $shipment_meta->get_data()['data'];
		} else {
			$shipment_data = [];
		}
	}else{
