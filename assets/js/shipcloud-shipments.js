...
var wcsc_translate = wcsc_translate || {};
...
shipcloud.ShipmentModel = Backbone.Model.extend({
            ...
            _(
                {
					...
					'error'	 : self.printErrors.bind(self)
                }
            ).extend(options)
        );
    },
    ...
    destroy: function (force_destroy = false) {
		...
		if ( force_destroy ) {
			...
			console.log("force_destroy "+shipment_id+" called");
			var dialog = jQuery('#ask-delete-shipment').dialog({
							...
							jQuery.post(
								...
								function (response) {
									if (response.success == false) {
										self.printErrors(response.data.data);
									} else {
			...
			console.log("destroy "+shipment_id+" called");
			jQuery.post(
				ajaxurl,
				{
...
shipcloud.ShipmentAdditionalServicesView = wp.Backbone.View.extend({
	...
	addHandlers: function (shipmentId = false) {
		...
		$(prefix + "input[name='shipment[additional_services][dhl_parcel_outlet_routing][checked]']").change(function () {
			if ($(this).prop('checked')) {
				$(prefix + '.shipcloud_dhl_parcel_outlet_routing').fadeIn();
			} else {
				$(prefix + '.shipcloud_dhl_parcel_outlet_routing').fadeOut();
			}
		});
		
		$(prefix + "input[name='shipment[additional_services][dhl_endorsement][checked]']").change(function () {
			if ($(this).prop('checked')) {
				$(prefix + '.shipcloud_dhl_endorsement').fadeIn();
			} else {
				$(prefix + '.shipcloud_dhl_endorsement').fadeOut();
			}
		});
		
		$(prefix + "input[name='shipment[additional_services][pickup_object][checked]']").prop('checked', true);
		$(prefix + "input[name='shipment[additional_services][pickup_object][checked]']").click(function(e) {
			e.preventDefault();
			e.stopPropagation();
		});
	...
	handleAdditionalServices: function (carrier, shipmentId = false) {
		...
		var allowed_additional_services_for_carrier = model.allowedAdditionalServices(); //[carrier];
	  
		allowed_additional_services_for_carrier = allowed_additional_services_for_carrier[carrier];
		...
		if (undefined != allowed_additional_services_for_carrier && allowed_additional_services_for_carrier.length > 0) {
	...
	handleAdvanceNotice: function (carrier, shipmentId = false) {
		...
		if (_.contains(this.model.allowedAdditionalServices()[carrier], 'advance_notice')) {
	        ...
	        var additional_services = this.model.allowedAdditionalServices()[carrier];
			if (additional_services.length == 1) {
				// special case when the contract only provides advance notice 
				// as additional service and customer has declined email notifications
				$('.shipcloud_additional_service__only_advance_notice').show();
			} else {
				$('.shipcloud_additional_service__only_advance_notice').hide();
			}
    ...
    renderLabelFormats: function (el) {
		...
		jQuery(carrier_data['label_formats']).each(function (index, value) {
			...
			jQuery(label_formats_for_service).each(function (index, value) {
				...
				console.log( "model.get('label').format: " + model.get('label').format );
